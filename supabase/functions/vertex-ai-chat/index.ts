  ...
  async executeWithRetry<T>(operation: () => Promise<T>, context: string): Promise<T> {
    ...
    for (let attempt = 1; attempt <= this.retryConfig.maxRetries; attempt++) {
      try {
        ...
        console.log(`vertex-ai-chat - Attempt ${attempt}/${this.retryConfig.maxRetries} failed in ${context}:`, error);
...
async function handleChatRequest(request: Request): Promise<Response> {
  ...
  try {
    ...
    try {
      ...
      console.error('vertex-ai-chat - Request parsing error:', parseError);
    ...
    console.error(`vertex-ai-chat - Critical error:`, error);
...
# Enhanced Python execution for vertex-ai-chat
...
`,
          context: context,
          message: message,
          function_source: "vertex-ai-chat",
          enhanced_mode: true
        }
      });
  ...
  return {
    content: `Hello! I'm your enhanced Vertex Ai Chat assistant. I'm equipped with advanced capabilities including:

🤖 **AI Processing**: Powered by Google Vertex gemini-1.5-pro
...
serve(async (req: Request) => {
  ...
  console.log(`vertex-ai-chat - ${method} request at ${timestamp}`);
...
console.log(`🚀 vertex-ai-chat started successfully with enhanced infrastructure`);
console.log(`🔧 Provider: Google Vertex | Model: gemini-1.5-pro`);
