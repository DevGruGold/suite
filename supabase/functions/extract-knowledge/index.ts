...
serve(async (req) => {
  ...
  try {
    ...
    if (content && message_id) {
      console.log(`🔍 Direct Mode: Extracting knowledge from message ${message_id}...`);
      const result = await processContent(supabase, content, message_id, session_id);

      await usageTracker.success({ ...result, mode: 'direct' });
    ...
    console.log('⏰ Cron Mode: Fetching pending NEWS from eliza_activity_log...');
...
Original Story: ${item.metadata?.original_story || ''}
    ...
    console.log(`✅ Cron run complete. Processed ${processedCount} news items.`);
