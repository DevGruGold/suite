  ...
  async executeWithRetry<T>(operation: () => Promise<T>, context: string): Promise<T> {
    ...
    for (let attempt = 1; attempt <= this.retryConfig.maxRetries; attempt++) {
      try {
        ...
        console.log(`openai-chat - Attempt ${attempt}/${this.retryConfig.maxRetries} failed in ${context}:`, error);
...
async function handleChatRequest(request: Request): Promise<Response> {
  ...
  try {
    ...
    try {
      ...
      console.error('openai-chat - Request parsing error:', parseError);
    ...
    console.error(`openai-chat - Critical error:`, error);
...
# Enhanced Python execution for openai-chat
...
`,
  ...
  return {
    content: `Hello! I'm your enhanced Openai Chat assistant. I'm equipped with advanced capabilities including:

🤖 **AI Processing**: Powered by OpenAI gpt-4
...
serve(async (req: Request) => {
  ...
  console.log(`openai-chat - ${method} request at ${timestamp}`);
...
console.log(`🚀 openai-chat started successfully with enhanced infrastructure`);
